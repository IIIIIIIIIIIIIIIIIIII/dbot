...
@ScriptManifest(author = "Michael W", category = Category.MISC, description = "Blast Furnace B0t", name = "Blast Furnace 1-99", version = 4.0)
public class Main extends AbstractScript {
	...
	public void onStart(){
		...
		closeButton = getWidgets().getWidgetChild(28, 119);
    	...
    	realTime = new Timer();
		 if(getSkills().getRealLevel(Skill.SMITHING) < 60){
			 ...
			 needsToPay = true;
			 log("under 60 acc, will pay foreman");
	        	payForeman();
	        	log("payforeman ran");
	       }
		 else if(getSkills().getRealLevel(Skill.SMITHING) >= 60){
			 log("took the wrong execution");
			 needsToPay = false;
			 State = 0;
		 }
	...
	public int onLoop() {
		...
		//<60 10 min payment logic
		if((realTime.elapsed() > 60000) && needsToPay == true){
			tempState = State;
			State = 8;
			realTime.reset();
		}
	...
	//foreman implementation for those under level 60 Smithing
		...
		public void payForeman(){
	       ...
	       if(tempState == 6){	
	    	   ...
	    	   ironButton.interact();
				...
				sleepUntil(() -> !closeButton.isVisible(), 15000);
		    	   log("state 6 execuuuuuted");


	       }
	        foreman = getNpcs().closest("Blast Furnace Foreman");
			if(foreman != null){
				
				log("foreman not null");
				getCamera().rotateToEntity(foreman);
				sleepUntil(() -> foreman.isOnScreen(), 15000);
